...
DEFAULT_FILE = "2025-12-29.1766986424.606828104.t4433"
...
class IcaoGraphs:
    def __init__(self, alt_dict, spd_dict, pos_dict, course_dict, adsb_icao_list, icao_callsigns, 
                 ...
                 icao_airspeed_ts=None, icao_status=None, icao_acas=None, icao_target_state=None, 
                 icao_operation_status_airborne=None, icao_operation_status_surface=None,
                 icao_acq_ts=None):
        ...
        self.icao_airspeed_ts = icao_airspeed_ts if icao_airspeed_ts else {}
        self.icao_status = icao_status if icao_status else {}
        self.icao_acas = icao_acas if icao_acas else {}
        self.icao_target_state = icao_target_state if icao_target_state else {}
        self.icao_operation_status_airborne = icao_operation_status_airborne if icao_operation_status_airborne else {}
        self.icao_operation_status_surface = icao_operation_status_surface if icao_operation_status_surface else {}
        ...
        self.plot_modes = ['altitude', 'speed', 'altitude_speed_combined', 
                           ...
                           'reg09_1_msg_intervals', 'reg09_3_msg_intervals', 'reg61_1_msg_intervals', 
                           'reg61_2_msg_intervals', 'reg62_msg_intervals', 'reg65_1_msg_intervals', 
                           'reg65_2_msg_intervals', 'df11_msg_intervals']
    ...
    def plot_current(self):
        ...
        # edit this part
            ...
            if not data_to_plot:
                self.ax.text(0.5, 0.5, f"Нет данных о путевой скорости при нахождении в воздухе для {icao}", ha='center', va='center')
                self.has_plot_data = False
                ...
                self.ax.set_title(f'Распределение интервалов сообщений сквиттера путевой скорости при нахождении в воздухе: {display_id} (REG09)')
                self.ax.legend(
                    title=f"Всего интервалов {len(intervals)}"
                )
                self.ax.grid(True, linestyle='--', alpha=0.7)
        ...
        elif mode == 'reg09_3_msg_intervals':
            data_to_plot = []
            
            if icao in self.icao_airspeed_ts and len(self.icao_airspeed_ts[icao]) > 1:
                timestamps = np.array(sorted(self.icao_airspeed_ts[icao]))
                intervals = np.diff(timestamps) * 1000
                intervals = intervals[intervals >= 0]
                if len(intervals) > 0:
                    data_to_plot.append((intervals, 'lime'))
            ...
            if not data_to_plot:
                self.ax.text(0.5, 0.5, f"Нет данных о воздушной скорости при нахождении в воздухе для {icao}", ha='center', va='center')
                self.has_plot_data = False
                ...
                self.ax.set_xlabel('Интервал между сообщениями (мс)')
                self.ax.set_ylabel('Частота встречаемости сообщения')
                self.ax.set_title(f'Распределение интервалов сообщений сквиттера воздушной скорости при нахождении в воздухе: {display_id} (REG09)')
                self.ax.legend(
                    title=f"Всего интервалов {len(intervals)}"
                )
                self.ax.grid(True, linestyle='--', alpha=0.7)
        ...
        # edit this part
        ...
        elif mode == 'reg61_1_msg_intervals':
            data_to_plot = []
        ...
        elif mode == 'reg61_2_msg_intervals':
            data_to_plot = []
            
            if icao in self.icao_acas and len(self.icao_acas[icao]) > 1:
                timestamps = np.array(sorted(self.icao_acas[icao]))
                intervals = np.diff(timestamps) * 1000
                intervals = intervals[intervals >= 0]
                if len(intervals) > 0:
                    data_to_plot.append((intervals, 'darkviolet'))
            ...
            if not data_to_plot:
                self.ax.text(0.5, 0.5, f"Нет данных о работе БСПС воздушного судна для {icao}", ha='center', va='center')
                self.has_plot_data = False
                ...
                self.ax.set_xlabel('Интервал между сообщениями (мс)')
                self.ax.set_ylabel('Количество')
                self.ax.set_title(f'Распределение интервалов сообщений сквиттера работы БСПС {display_id} (REG61)')
        ...
        elif mode == 'reg65_1_msg_intervals':
            data_to_plot = []
            
            if icao in self.icao_operation_status_airborne and len(self.icao_operation_status_airborne[icao]) > 1:
                timestamps = np.array(sorted(self.icao_operation_status_airborne[icao]))
                intervals = np.diff(timestamps) * 1000
                intervals = intervals[intervals >= 0]
                if len(intervals) > 0:
            ...
            if not data_to_plot:
                self.ax.text(0.5, 0.5, f"Нет данных об эксплуатационном статусе в полете для {icao}", ha='center', va='center')
                self.has_plot_data = False
                ...
                self.ax.set_xlabel('Интервал между сообщениями (мс)')
                self.ax.set_ylabel('Количество')
                self.ax.set_title(f'Распределение интервалов сообщений сквиттера эксплуатационного статуса в полете {display_id} (REG65)')
        ...
        elif mode == 'reg65_2_msg_intervals':
            data_to_plot = []
            
            if icao in self.icao_operation_status_surface and len(self.icao_operation_status_surface[icao]) > 1:
                timestamps = np.array(sorted(self.icao_operation_status_surface[icao]))
                intervals = np.diff(timestamps) * 1000
                intervals = intervals[intervals >= 0]
                if len(intervals) > 0:
                    data_to_plot.append((intervals, 'mediumaquamarine'))
            ...
            if not data_to_plot:
                self.ax.text(0.5, 0.5, f"Нет данных об эксплуатационном статусе на земле для {icao}", ha='center', va='center')
                self.has_plot_data = False
                ...
                self.ax.set_xlabel('Интервал между сообщениями (мс)')
                self.ax.set_ylabel('Количество')
                self.ax.set_title(f'Распределение интервалов сообщений сквиттера эксплуатационного статуса на земле {display_id} (REG65)')
        ...
        elif mode == 'df11_msg_intervals':
            data_to_plot = []
            
            if icao in self.icao_df11_ts and len(self.icao_df11_ts[icao]) > 1:
                timestamps = np.array(sorted(self.icao_df11_ts[icao]))
                intervals = np.diff(timestamps) * 1000
                intervals = intervals[intervals >= 0]
                if len(intervals) > 0:
                    data_to_plot.append((intervals, 'orange'))

            if not data_to_plot:
                self.ax.text(0.5, 0.5, f"Нет данных об опознавании {icao}", ha='center', va='center')
                self.has_plot_data = False
                ...
                self.ax.set_xlabel('Интервал между сообщениями (мс)')
                self.ax.set_ylabel('Количество')
                self.ax.set_title(f'Распределение интервалов сообщений сквиттера опознавания {display_id} (DF 11)')
...
if __name__ == '__main__':
    ...
    icao_ground_speed_ts = {}
    icao_airspeed_ts = {}
    icao_status_ts = {}
    icao_acas_ts = {}
    icao_target_state_ts = {}
    icao_operation_status_airborne_ts = {}
    icao_operation_status_surface_ts = {}
    ...
    try:
        ...
        with open(file_path, "r") as f:
            for line_num, line in enumerate(f, 1):
                ...
                try:
                    ...
                    if df == 11:
                        icao_acquisition_ts.setdefault(aa, []).append(timestamp)
                        continue
                ...
                if df not in [17, 18]: continue # только ads-b сообщения
                ...
                try:
                    ...
                    # if tc == 19:
                    #     icao_ground_speed_ts.setdefault(aa, []).append(timestamp)

                    if tc == 19:
                        msg_bin = pms.common.hex2bin(message_str)
                        subtype = pms.common.bin2int(msg_bin[37:40])

                        icao_ground_speed_ts.setdefault(aa, []).append(timestamp)
                        icao_airspeed_ts.setdefault(aa, []).append(timestamp)
                    ...
                    if tc == 28:
                        ...
                        if subtype == 1:
                            ...
                            icao_acas_ts.setdefault(aa, []).append(timestamp)
                    ...
                    if tc == 31:
                        ...
                        if subtype == 0:
                            icao_operation_status_airborne_ts.setdefault(aa, []).append(timestamp)
                        elif subtype == 1:
                            icao_operation_status_surface_ts.setdefault(aa, []).append(timestamp)
        ...
        IcaoGraphs(icao_altitude, icao_speed, icao_positions, icao_courses, adsb_icao_list, 
                   ...
                   icao_ground_speed_ts, icao_airspeed_ts, icao_status_ts, icao_acas_ts, icao_target_state_ts, 
                   icao_operation_status_airborne_ts, icao_operation_status_surface_ts,
                   icao_acquisition_ts)
